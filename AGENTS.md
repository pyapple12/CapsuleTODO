...
**当前状态**：**V0.1.1.4（2026-09-26）——设计实验场 UI 初版并入**。ui-1.0-feature 分支 28 轮迭代（ui1.0 V0.001–V0.028）经 squash 合并进 main（单条提交；分支保留归档，见分支处理章）；实验场现状：`design/` 独立原型三页签全交互（纯 HTML/CSS/JS 零构建；index.html 仅存骨架，组件样式/脚本按功能拆至 `assets/css/`、`assets/js/` 共 22 文件，引用顺序 = 级联/执行顺序不得乱序，glass.css 令牌层留根），清单删除钮二态盖翻确认（盖翻+脉冲+离开即回退）、罩死行禁交互灰化、长按拖拽重挂模型，气泡单击开全文板/双击复制、拖拽与罩死同款推广，归档成品化（完成时间倒序/增量摘除）与边缘三角随板缩放生长，玻璃滑杆与整板阅读工厂五容器统一（到底抬带/半截行溶解）；二期（PL004–PL006 + FIX002 全部收口，2026-09-17）：页签导航与气泡、白板用户目验全过，A002 第 2 轮审计闭环（观察项延续保留观察），45 项测试全绿、门禁七项绿，V0.1.1.1 已推送；实验场原型期（PL007 全勾结，做完不审计——用户定案）：glass.css 玻璃引擎令牌化（基准配方读数），README 映射表（七区块 → Vue 组件 → 落点）与可行性标注，RESEARCH 沿系列同步；uiverse 组件五处引入（页签含徽章/输入框/勾选框/删除/录制，均用户目验定值），REC 玻璃配方推广全玻璃板（panel-bg 深度 ×2 + panel-shadow 落影令牌），清单分组虚拟外框动态公式（框宽 = 卡宽 − 20，行宽与其他面板等宽），配色基准 `design/color_spark.md`（暮色截图实测取色）；**下一步：主分支 APP 改造立项（实验场配方落回 ui/）**。三期 AI 规范待立项讨论（仅记录见计划书 §6）。沿系列基线：Tauri 2 + 纯 Rust 业务 + Vue 展示 + DWM 焦点联动玻璃材质（配方沿 CapsulePulse PL010/PL011 定案）；macOS/Linux 适配延后 [problems#1]。`.agents/skills/` 存放项目自建 skill（audit-project / audit-report / progress-task）。遗留与远期项登记 `y.problems.md`。
